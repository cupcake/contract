...
export async function getMetadataPDA(tokenMint: PublicKey) {
  return (
    await PublicKey.findProgramAddress(
      [Buffer.from('metadata'), TokenMetadata.PROGRAM_ID.toBuffer(), tokenMint.toBuffer()],
...
export async function getMasterEditionPDA(tokenMint: PublicKey) {
  return (
    await PublicKey.findProgramAddress(
      [Buffer.from('metadata'), TokenMetadata.PROGRAM_ID.toBuffer(), tokenMint.toBuffer(), Buffer.from('edition')],
      TokenMetadata.PROGRAM_ID
    )
  )[0];
}
...
export class CupcakeProgram {
  ...
  async bakeSprinkle(
    ...
    const metadata = await TokenMetadata.Metadata.fromAccountAddress(this.program.provider.connection, metadataPDA);
